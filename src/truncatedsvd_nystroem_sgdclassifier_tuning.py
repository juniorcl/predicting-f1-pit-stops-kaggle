...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/truncatedsvd_nystroem_sgdclassifier.log'), 
...
def objective(trial, X, y):
    ...
    for fold, (train_idx, valid_idx) in enumerate(cv.split(X, y)):
        ...
        model = make_pipeline(
            column_transformer,
            TruncatedSVD(
                n_components=trial.suggest_int("trunsvd_n_components", 2, 75),
                ...
                tol=trial.suggest_float("trunsvd_tol", 1e-6, 1e-2, log=True)
            ),
            StandardScaler(),
            Nystroem(
...
best_params = {
    'trunsvd_n_components': 49, 
    'algorithm': 'randomized', 
    'n_iter': 7, 
    'power_iteration_normalizer': 'auto', 
    'trunsvd_tol': 0.0002007740868798078, 
    'kernel': 'rbf', 
    'gamma': 0.0015475955121722707, 
    'n_components': 72, 
    'loss': 'log_loss', 
    'penalty': 'l2', 
    'alpha': 4.077026481027349e-07, 
    'learning_rate': 'adaptive', 
    'eta0': 0.0011419601057602714, 
    'l1_ratio': 0.4519587541447113, 
    'tol': 0.00015615232993554682, 
    'average': True
} # study.best_params
...
dump_pickle(pipe_tuned, '../models/model_truncatedsvd_nystroem_sgdclassifier.pkl')
